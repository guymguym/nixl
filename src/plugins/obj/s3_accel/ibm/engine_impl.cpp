...
/**
 * Transfer request handle for Storage Scale operations.
 * Contains information needed for RDMA-accelerated S3 transfers.
 */
class IBMObjTransferRequestH {
public:
     ...
     */
    IBMObjTransferRequestH() : addr(0), size(0), offset(0), rdma_desc(""), obj_key("") {}
     ...
     */
    IBMObjTransferRequestH(uintptr_t a, size_t s, size_t off)
        : addr(a),
          size(s),
          offset(off),
          rdma_desc(""),
    ...
    ~IBMObjTransferRequestH() = default;
};

/**
 * Backend request handle for Storage Scale operations.
 * Manages multiple transfer requests and their completion status.
 */
class nixlIBMObjBackendReqH : public nixlBackendReqH {
public:
    /// Vector of transfer requests
    std::vector<IBMObjTransferRequestH> reqs_;
...
/**
 * Metadata for Storage Scale operations.
 * Contains information about registered memory regions.
 */
class nixlIBMObjMetadata : public nixlBackendMD {
public:
    /**
     ...
     */
    nixlIBMObjMetadata(nixl_mem_t nixl_mem, uint64_t dev_id, std::string obj_key)
        : nixlBackendMD(true),
          nixlMem(nixl_mem),
          devId(dev_id),
          objKey(obj_key),
     ...
     */
    nixlIBMObjMetadata(nixl_mem_t nixl_mem, uintptr_t addr)
        : nixlBackendMD(true),
          nixlMem(nixl_mem),
          devId(0),
          objKey(""),
...
/// cuObject I/O operations for Storage Scale
CUObjIOOps IBM_ops = {.get = objectGet, .put = objectPut};
} // namespace

/**
 * Constructor for S3IBMStorageScaleEngineImpl.
 * Initializes the S3 IBM Storage Scale engine with both S3 client and cuObject client.
 *
 * @param init_params Backend initialization parameters containing custom S3 configuration
 */
S3IBMStorageScaleEngineImpl::S3IBMStorageScaleEngineImpl(const nixlBackendInitParams *init_params)
    : S3AccelObjEngineImpl(init_params) {
    s3Client_ = std::make_shared<awsS3IBMClient>(init_params->customParams, executor_);
    NIXL_INFO << "Object storage backend initialized with S3 IBM Storage Scale client";

    cuClient_ = std::make_shared<cuObjClient>(IBM_ops, CUOBJ_PROTO_RDMA_DC_V1);
...
/**
 * Constructor for S3IBMStorageScaleEngineImpl with injected S3 client.
 ...
 */
nixl_status_t
S3IBMStorageScaleEngineImpl::registerMem(const nixlBlobDesc &mem,
    ...
    if (nixl_mem == OBJ_SEG) {
        std::unique_ptr<nixlIBMObjMetadata> obj_md = std::make_unique<nixlIBMObjMetadata>(
        ...
        NIXL_DEBUG << "registerMem: addr=" << mem.addr << ", len=" << mem.len
                   << ", nixl_mem=" << nixl_mem;
        std::unique_ptr<nixlIBMObjMetadata> mem_md =
            std::make_unique<nixlIBMObjMetadata>(nixl_mem, mem.addr);
 ...
 */
nixl_status_t
S3IBMStorageScaleEngineImpl::deregisterMem(nixlBackendMD *meta) {
    nixlIBMObjMetadata *md = static_cast<nixlIBMObjMetadata *>(meta);
    if (md) {
        if (md->nixlMem == OBJ_SEG) {
            std::unique_ptr<nixlIBMObjMetadata> obj_md_ptr =
                std::unique_ptr<nixlIBMObjMetadata>(md);
            devIdToObjKey_.erase(obj_md_ptr->devId);
        } else if ((md->nixlMem == DRAM_SEG) || (md->nixlMem == VRAM_SEG)) {
            std::unique_ptr<nixlIBMObjMetadata> mem_md_ptr =
                std::unique_ptr<nixlIBMObjMetadata>(md);
            ...
            if (cuda_status != CU_OBJ_SUCCESS) {
                ...
                // The nixlIBMObjMetadata remains valid for caller to retry or clean up
                mem_md_ptr.release();
                return NIXL_ERR_BACKEND;
            }
        }
 ...
 */
nixl_status_t
S3IBMStorageScaleEngineImpl::prepXfer(const nixl_xfer_op_t &operation,
                              const nixl_meta_dlist_t &local,
                              const nixl_meta_dlist_t &remote,
                              const std::string &remote_agent,
                              const std::string &local_agent,
    ...
    auto req_h = std::make_unique<nixlIBMObjBackendReqH>();

    for (int i = 0; i < local.descCount(); ++i) {
        IBMObjTransferRequestH req(local[i].addr, local[i].len, remote[i].addr);
 ...
 */
nixl_status_t
S3IBMStorageScaleEngineImpl::postXfer(const nixl_xfer_op_t &operation,
                              const nixl_meta_dlist_t &local,
                              const nixl_meta_dlist_t &remote,
                              const std::string &remote_agent,
                              nixlBackendReqH *&handle,
    ...
    nixlIBMObjBackendReqH *req_h = static_cast<nixlIBMObjBackendReqH *>(handle);
    ...
    for (const auto &req : req_h->reqs_) {
        ...
        auto rdmaClient = dynamic_cast<iIBMS3RdmaClient *>(s3Client_.get());
        if (!rdmaClient) {
            NIXL_ERROR << "IBM RDMA operations require iIBMS3RdmaClient";
 ...
 */
nixl_status_t
S3IBMStorageScaleEngineImpl::checkXfer(nixlBackendReqH *handle) const {
    if (handle == nullptr) {
        NIXL_ERROR << "transfer request handle is null";
        return NIXL_ERR_INVALID_PARAM;
    }
 ...
 */
nixl_status_t
S3IBMStorageScaleEngineImpl::releaseReqH(nixlBackendReqH *handle) const {
    if (handle == nullptr) {
        NIXL_ERROR << "transfer request handle is null";
        return NIXL_ERR_INVALID_PARAM;
    }
 ...
 */
iS3Client *
S3IBMStorageScaleEngineImpl::getClient() const {
