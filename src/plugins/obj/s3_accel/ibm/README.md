...
# NIXL IBM Storage Scale accelerated S3 engine

This vendor-specific accelerated engine provides S3 over RDMA for IBM Storage Scale.  This engine utilizes the CUDA Toolkit cuObject Client library and the AWS S3 SDK.

If a IBM Storage Scale endpoint is utilized, but RDMA is not enabled, the standard S3 engine should be used instead.
...
To enable the IBM Storage Scale engine, the `accelerated` parameter must be set to `true` and the `type` parameter must be set to `ibm`.  It is recomended that the `req_checksum` parameter be set to `required` to ensure data integrity.

### Configuration Examples

#### IBM Storage Scale Endpoint
...
nixl_b_params_t params = {
    ...
    {"secret_key", "secret"},
    {"bucket", "test-bucket"},
    {"endpoint_override", "http://10.0.0.1:6001"},
    {"scheme", "http"},
...
```json
{
    ...
    "properties": {
        ...
        "rdma_dev_addr_list": ["10.20.30.1", "10.20.30.2"]
    }
}
```
...
```bash
export CUFILE_ENV_PATH_JSON=/path/to/cuobj.json
```

## Supported Memory Types

The IBM Storage Scale engine supports the following memory types:
...
The IBM Storage Scale Object Storage backend supports read and write operations between local CPU or GPU memory and S3 objects. Here are the key aspects of transfer operations:
...
- The local memory buffer is written using RDMA by the Storage Scale endpoint using the requested object data
...
- The local memory buffer is read using RDMA by the Storage Scale endpoint and persisted to object storage
